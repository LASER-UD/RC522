...
void PCD_ReadRegisters(	unsigned char addr,	///< The register to read from. One of the PCD_Register enums.
	...
	if (rxAlign) {		// Only update bit positions rxAlign..7 in values[0]
        ...
        while ( ! (SPI_ReadTxStatus() & SPI_STS_SPI_IDLE) );
        SPI_WriteTxData(((addr<<1)&0x7E)|0x80);// Equivalente a SPI.transfer(((addr<<1)&0x7E) | 0x80);
		unsigned char value =  SPI_ReadRxData();
	...
	while (index < count) {
        while ( ! (SPI_ReadTxStatus() & SPI_STS_SPI_IDLE) );
		SPI_WriteTxData(((addr<<1)&0x7E)|0x80);// Equivalente a SPI.transfer(((addr<<1)&0x7E) | 0x80);
		values[index] = SPI_ReadRxData();
        index++;
	}
    SPI_WriteTxData(0x00);
	values[index] = SPI_ReadRxData(); // Equivalente a val = SPI.transfer(0x00);
